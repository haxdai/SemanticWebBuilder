...
namespace WBWord
{
	...
	public class NewStep6 : TSWizards.BaseStep
	{
		...
		{			
			...
			Parameter parametro= new Parameter("title",step2.txttitulo.Text);  
			FrmNewContent.parametros.Add(parametro);  			

			NewStep3 step3 = Wizard.GetStep("Step3") as NewStep3;
			parametro= new Parameter("description",step3.txtDesc.Text);
			FrmNewContent.parametros.Add(parametro);
			
			
			NewStep8 step8=Wizard.GetStep("Step8") as NewStep8;
			if(step8.checkBoxConfPaginacion.Checked)
			{
				FrmNewContent.parametros.Add(new Parameter("createPages","true"));						
				FrmNewContent.parametros.Add(new Parameter("pages",step8.numericUpNumero.Value.ToString()));						
				FrmNewContent.parametros.Add(new Parameter("stxtsig",step8.txtTextoSiguiente.Text));						
				FrmNewContent.parametros.Add(new Parameter("stxtant",step8.textBoxAnterior.Text));						
				if(step8.radioButtonAbajo.Checked)
					FrmNewContent.parametros.Add(new Parameter("position","1"));						
				if(step8.radioButtonArriba.Checked)
					FrmNewContent.parametros.Add(new Parameter("position","2"));						
				if(step8.radioButtonAmbos.Checked)
					FrmNewContent.parametros.Add(new Parameter("position","3"));												
			}
			...
			if(treeView1.SelectedNode!=null && treeView1.SelectedNode is WBNode)
			{
				...
				if(CWebBuilder.user.GetPermiso(idtopicmap,CWebBuilder.TYPE))
				{
					bool existe=false;
					...
					if(!existe)
					{
						...
						Parameter ptopicmap=new Parameter("topicmap",idtopicmap); 
						FrmNewContent.parametros.Add(ptopicmap); 
					}
					Servicios s= new Servicios(CWebBuilder.user);
					FrmNewContent.parametros.Add(new WBOffice3.Parameter("type",CWebBuilder.TYPE));
					...
					foreach(IParameter paramres in FrmNewContent.resultados)
					{
						if(paramres.Name.Equals("id"))
						{
							idcontent=paramres.Value;
							object id=paramres.Value;
							foreach(FileInfo fileimagen in FrmNewContent.imagenes)
							{
								String resType="LocalContent";
								CContentUpload cfile=new CContentUpload(fileimagen,idtopicmap,paramres.Value,"1",CWebBuilder.user,resType);
							...
							try
							{
								object path=CWebBuilder.doc.FullName;
								
								object savechanges=Word.WdSaveOptions.wdSaveChanges; 
								object original=Word.WdOriginalFormat.wdWordDocument;
								...
								if(step8.checkBoxConfPaginacion.Checked)
								{
									String conf=step8.numericUpNumero.Value.ToString();
									conf+="|"+step8.txtTextoSiguiente.Text;						
									conf+="|"+step8.textBoxAnterior.Text;						
									if(step8.radioButtonAbajo.Checked)
										conf+="|1";					
									if(step8.radioButtonArriba.Checked)
										conf+="|2";															
