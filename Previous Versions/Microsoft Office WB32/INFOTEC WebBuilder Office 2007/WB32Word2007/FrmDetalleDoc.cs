...
namespace WBWord
{
	...
	public class FrmDetalleDoc : System.Windows.Forms.Form
	{
		...
		public FrmDetalleDoc()
		{
			...
			FileInfo fdocword= new FileInfo(CWebBuilder.doc.FullName);			

			itemsdoc[0]=resources.GetString("FrmDetalleDoc.msg1");
			itemsdoc[1]=CWebBuilder.doc.Name;
			System.Windows.Forms.ListViewItem itemdoc=new ListViewItem(itemsdoc);
			this.listView1.Items.Add(itemdoc);  

			itemsdoc[0]=resources.GetString("FrmDetalleDoc.msg2");			
			itemsdoc[1]=fdocword.Directory.FullName ; 
			itemdoc=new ListViewItem(itemsdoc);
			this.listView1.Items.Add(itemdoc);  


			itemsdoc[0]=resources.GetString("FrmDetalleDoc.msg3");			
			itemsdoc[1]=""+CWebBuilder.doc.Words.Count  ; 
			itemdoc=new ListViewItem(itemsdoc);
			this.listView1.Items.Add(itemdoc);  

			itemsdoc[0]=resources.GetString("FrmDetalleDoc.msg4");			
			itemsdoc[1]=""+(CWebBuilder.doc.InlineShapes.Count+CWebBuilder.doc.Shapes.Count) ; 
			itemdoc=new ListViewItem(itemsdoc);
			this.listView1.Items.Add(itemdoc);  

			
			itemsdoc[0]=resources.GetString("FrmDetalleDoc.msg5");			
			itemsdoc[1]=""+CWebBuilder.doc.Hyperlinks.Count;  
			itemdoc=new ListViewItem(itemsdoc);
			this.listView1.Items.Add(itemdoc);  


			itemsdoc[0]=resources.GetString("FrmDetalleDoc.msg6");			
			itemsdoc[1]=fdocword.Length+" bytes";
			itemdoc=new ListViewItem(itemsdoc);
			this.listView1.Items.Add(itemdoc);  

			itemsdoc[0]=resources.GetString("FrmDetalleDoc.msg7");			
			itemsdoc[1]=this.ValidaNombre(fdocword);
			itemdoc=new ListViewItem(itemsdoc);
			this.listView1.Items.Add(itemdoc);  
			if(itemsdoc[1]!=resources.GetString("FrmDetalleDoc.msg8"))
			{
						...
						try
						{
							...
							if(!filepath.IsFile )
							{
								...
								try
								{
									...
									if(CWebBuilder.user.ServerProxy!=null && CWebBuilder.user.ServerProxy!="")
									{
										// por probar
										wbpagina.Proxy=new WebProxy(Uri.UriSchemeHttp+CWebBuilder.user.ServerProxy+":"+CWebBuilder.user.PortServerProxy);
									}
								...
								if(existe)
								{
									itemspag[1]=resources.GetString("FrmDetalleDoc.si");
								}
								else
								{
									itemspag[1]=resources.GetString("FrmDetalleDoc.no");
								}
								System.Windows.Forms.ListViewItem itempag= new ListViewItem(itemspag);
								if(itemspag[1]==resources.GetString("FrmDetalleDoc.si"))
								{
		...
		private String ValidaNombre(FileInfo archivo)
		{
			...
			if(nombre.Length>40)
			{
				return resources.GetString("FrmDetalleDoc.msg9");				
			}
			...
			for(int i=0;i<letras.Length;i++)
			{
				...
				{
					return resources.GetString("FrmDetalleDoc.msg10")+": "+ letra +"";
					
				}
				else if(Char.IsWhiteSpace(letra)) 
				{
					return resources.GetString("FrmDetalleDoc.msg11");
					
				}
				else if (!Char.IsLetterOrDigit(letra))
				{
					return resources.GetString("FrmDetalleDoc.msg13") +": "+ letra + "";
			...
			return resources.GetString("FrmDetalleDoc.msg8");
		}
