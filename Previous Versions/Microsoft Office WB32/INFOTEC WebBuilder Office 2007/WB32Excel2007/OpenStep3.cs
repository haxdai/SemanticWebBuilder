	...
	public class OpenStep3 : TSWizards.BaseStep
	{
		...
		private void OpenStep3_ShowStep(object sender, TSWizards.ShowStepEventArgs e)
		{
			...
			if(step2.treeView1.SelectedNode!=null && step2.treeView1.SelectedNode is WBNode)
			{
				...
				foreach(System.Xml.XmlElement  xmlcontrel in xmlcontents)
				{
					...
					Servicios sAtt=new Servicios(CExcel.user);
					XmlDocument doxAtt=sAtt.getContentAtt(idtopicmap,topicid,idcontent);
					if(doxAtt.GetElementsByTagName("nameType").Count>0)
					{
						String nameType=doxAtt.GetElementsByTagName("nameType")[0].InnerText;
		...
		private void listView1_DoubleClick(object sender, System.EventArgs e)
		{
			foreach(ListViewItem item in this.listView1.SelectedItems)
			{
				...
				FrmPreview frm=new FrmPreview(idtopicmap,idtopic,contentid,version,CExcel.user);
				frm.ShowDialog(this);
				return;
			}
		}
