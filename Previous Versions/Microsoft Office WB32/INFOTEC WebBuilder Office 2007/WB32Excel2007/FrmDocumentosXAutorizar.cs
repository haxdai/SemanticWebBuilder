...
namespace WBExcel
{
	...
	{
		static ResourceManager resources = new ResourceManager(typeof(CLibrary));
		...
		public FrmDocumentosXAutorizar(CUserAdmin user,Excel._Application app)
		{
			//
		...
		private void InitializeComponent()
		{
			...
			//this.axWebBrowser1.BackgroundImage = ((System.Drawing.Image)(resources.GetObject("axWebBrowser1.BackgroundImage")));			
			this.axWebBrowser1.Dock = ((System.Windows.Forms.DockStyle)(resources.GetObject("axWebBrowser1.Dock")));
			//this.axWebBrowser1.Enabled = ((bool)(resources.GetObject("axWebBrowser1.Enabled")));
			...
			//this.axWebBrowser1.Text = resources.GetString("axWebBrowser1.Text");
		...
		private void listView1_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			{				
				...
				/*Servicios s=new Servicios(this.user);				
				String url=s.getURLPreview(topicmap,topicid,idcont);
				if(lastversion!=null)
				{
					url+="?wblastversion="+lastversion+"&"+"_wb_username="+  user.User +"&_wb_password="+ user.Password;
				}	
				...
				object missing=Type.Missing;*/
				object missing=Type.Missing;
				FrmPreview frm=new FrmPreview(topicmap,topicid,idcont,lastversion,CExcel.user);
		...
		private void buttonInfo_Click(object sender, System.EventArgs e)
		{
			if(this.listView1.SelectedItems.Count>0)
			{
				...
				String topicmap=item.TopicMap;
				String topicid=item.Topic;			
				FrmProp prop=new FrmProp(topicmap,topicid,item.ID,this.user,CExcel.TYPE);
				prop.ShowDialog(this);
			}
		}
		/// <summary>
		/// Abre documento en formato excel
		/// </summary>
		/// <param name="app">Aplicación Excel</param>
		/// <param name="idtopicmap">Sitio</param>
		/// <param name="idtopic">Sección</param>
		/// <param name="idcontent">Contenido</param>
		/// <param name="user">Usuario</param>
		/// <param name="dinfo">Directorio de alamcenamiento</param>
		public static void OpenExcelDocument(Excel._Application app,String idtopicmap,String idtopic,String idcontent,CUserAdmin user,System.IO.DirectoryInfo dinfo)
		{
			String idversion="1";
			...
			try
			{
				if(resp.GetElementsByTagName("lastversion").Count>0)
				{
					idversion=resp.GetElementsByTagName("lastversion")[0].InnerText;
				}							
			...
			parametros.Add(new WBOffice3.Parameter("version",idversion)); 
			bool cont=true;
			while(cont)
			{
				
				System.Windows.Forms.DialogResult respDirTargetTest=DirTargetTest(dinfo.FullName);
				if(respDirTargetTest==System.Windows.Forms.DialogResult.OK)
				{
					cont=false;
				}
				if(respDirTargetTest==System.Windows.Forms.DialogResult.Cancel)
				{
					cont=false;				
					return;
				}					
			...
			foreach(WBOffice3.IParameter presult in resultados)
			{
				if(presult.Name.Equals("content"))
				{
					...
					foreach(Excel.Workbook docopen in app.Workbooks)
					{
						...
						{	
							if(idcont!=null && version!=null)
							{
								...
								if(f.Name.ToLower().EndsWith(".html") || f.Name.ToLower().EndsWith(".htm"))
								{
									...
									htmlimg=ParseaExcel(htmlimg,fdoc.Directory.FullName+"\\"+snamedoc+"\\",false,fdoc);
					...
					html=html.Replace("\r"," ");
					html=html.Replace("\n"," ");
					html=ParseaExcel(html,fdoc.Directory.FullName+"\\"+snamedoc+"\\",true,fdoc);
					...
					if(fExcel.Exists)
					{
                        Excel.Workbook dochtml = app.Workbooks.Open((string)fExcel.FullName, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);						
					...
					{
                        Excel.Workbook dochtml = app.Workbooks.Open((string)strpathHtml, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);						
						// convierte el archivo en formato Word
                        dochtml.SaveAs((string)strpathDoc, Excel.XlFileFormat.xlExcel7, missing, missing, objfalse, missing, Excel.XlSaveAsAccessMode.xlShared, missing, missing, missing, missing, missing);
						dochtml.Close(true,missing,missing);
					}
                    ...
                    Excel.WorkbookClass docopenwb = (Excel.WorkbookClass) app.Workbooks.Open((string)strpathDoc, missing, missing, missing, missing, missing, missing, missing, missing, missing, missing, missing, missing, missing, missing);
		...
		private void buttonOpen_Click(object sender, System.EventArgs e)
		{
			...
			{				
				...
				{			
					...
					try
					{										
						OpenExcelDocument(this.app,idtopicmap,idtopic,idcontent,this.user,new DirectoryInfo(treedir.FolderName));
					}
					catch(Exception ue)
					{
						Console.WriteLine(ue.StackTrace);
					}
					return;
				}
			}
		...
		private static String ParseaExcel(CHtmlParser.CTag tag,String attname,String html,String pathdoc,FileInfo fdoc)
		{
		...
		private static String ParseaExcel(String html,String pathdoc,bool showWarning,FileInfo fdoc)
		{
			bool warning=true;
			...
			foreach(CHtmlParser.CTag tag in doc.Tags)
			{
				...
				html=ParseaExcel(tag,"background",html,pathdoc,fdoc);
				html=ParseaExcel(tag,"codebase",html,pathdoc,fdoc);				
				html=ParseaExcel(tag,"src",html,pathdoc,fdoc);
				html=ParseaExcel(tag,"href",html,pathdoc,fdoc);
			...
			{
				MessageBox.Show(resources.GetString("FrmOpen3.msg1excel"),resources.GetString("Global.title"),MessageBoxButtons.OK,MessageBoxIcon.Warning);  
			}
			return html;
		}
		private static System.Windows.Forms.DialogResult DirTargetTest(String path)
		{
			try
			{
			...
			catch(System.IO.IOException ioedir)
			{
				...
				return MessageBox.Show(ioedir.Message,resources.GetString("Global.title"),MessageBoxButtons.RetryCancel,MessageBoxIcon.Warning);
			}
