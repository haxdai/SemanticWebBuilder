		...
		private void NewStep6_ShowStep(object sender, TSWizards.ShowStepEventArgs e)
		{
			...
			if(treeView1.SelectedNode!=null && treeView1.SelectedNode is WBNode)
			{
				...
				if(CPowerPoint.user.GetPermiso(idtopicmap,CPowerPoint.TYPE))
				{
					...
					if(step8.checkBoxDownload.Checked)
					{
						FrmNewContent.parametros.Add(new WBOffice3.Parameter("link","true"));
						FrmNewContent.parametros.Add(new WBOffice3.Parameter("text",step8.txtDownloadText.Text));
					}
					else
					{
						FrmNewContent.parametros.Add(new WBOffice3.Parameter("link","false"));
					}					
					FrmNewContent.parametros.Add(new WBOffice3.Parameter("type","powerpoint"));
					...
					foreach(IParameter paramres in FrmNewContent.resultados)
					{
						if(paramres.Name.Equals("id"))
						{
							...
							{                                
								...
								CContentUpload cfile=new CContentUpload(fileimagen,idtopicmap,paramres.Value,"1",CPowerPoint.user,resType);
							...
							try
							{
								object path=CPowerPoint.doc.FullName;
								...
								if(step8.checkBoxDownload.Checked)
								{
									String conf=step8.txtDownloadText.Text;
                                    props.Add("link", false, Office.MsoDocProperties.msoPropertyTypeString, conf, nullObjStr);							  							
								}
                                CPowerPoint.doc.Saved = Office.MsoTriState.msoTriStateMixed;                                
