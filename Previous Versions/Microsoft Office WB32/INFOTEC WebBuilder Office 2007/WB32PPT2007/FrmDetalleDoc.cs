...
namespace WBPowerPoint
{
	...
	public class FrmDetalleDoc : System.Windows.Forms.Form
	{
		...
		public FrmDetalleDoc()
		{
			...
			FileInfo fdocword= new FileInfo(CPowerPoint.doc.FullName);			

			itemsdoc[0]=resources.GetString("FrmDetalleDoc.msg1");
			itemsdoc[1]=CPowerPoint.doc.Name;
			...
			itemsdoc[1]=""+CExcel.doc.Words.Count  ; 
			itemdoc=new ListViewItem(itemsdoc);
			this.listView1.Items.Add(itemdoc);  */

			/*itemsdoc[0]=resources.GetString("FrmDetalleDoc.msg4");			
			itemsdoc[1]=""+(CExcel.doc.InlineShapes.Count+CWebBuilder.doc.Shapes.Count) ; 
			itemdoc=new ListViewItem(itemsdoc);
			this.listView1.Items.Add(itemdoc);  */

			
			/*itemsdoc[0]=resources.GetString("FrmDetalleDoc.msg5");			
			itemsdoc[1]=""+CExcel.doc.Hyperlinks.Count;  
			itemdoc=new ListViewItem(itemsdoc);
			this.listView1.Items.Add(itemdoc);  */
			...
			try
			{
				...
				foreach(PowerPoint. Slide slide in CPowerPoint.doc.Slides)
				{					
					foreach(PowerPoint.Hyperlink link in  slide.Hyperlinks )
					{
						...
						if(archivo!=null)
						{
							try
							{
								System.Uri basepath=new System.Uri(CPowerPoint.doc.FullName);
