...
namespace WBOffice3
{
	/// <summary>Control que muestra el resumen de actualización de un contenido</summary>
	public class Resumen1 : TSWizards.BaseExteriorStep
	{
		...
		CUserAdmin admin;
		/// <summary>Construye la forma en base a un usuario</summary>
		/// <param name="admin">Usuario que esta actualizando</param>
		public Resumen1(CUserAdmin admin)
		{
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(Resumen1));
			...
			this.ShowStep += new TSWizards.ShowStepEventHandler(this.Resumen1_ShowStep);
		...
		private void Resumen1_ShowStep(object sender, TSWizards.ShowStepEventArgs e)
		{
			String lastversion=null;
			string [] item=new String[3];
			int ifiles=0;
			int itotaltam=0;
			foreach(IParameter p in FrmResumenContenido.parametros)
			{
				if(p.IsFile)
				{
			...
			String id=null;
			foreach(IParameter pres in FrmResumenContenido.resultados)
			{
			...
			FrmResumenContenido.resultados.Add(pid);  
			listitem1= new ListViewItem(items2);
			this.listView2.Items.Add(listitem1); 
			foreach(IParameter pres in FrmResumenContenido.resultados)
			{
		...
		private void buttonVer_Click(object sender, System.EventArgs e)
		{
			
			String idcont="";
			String topicid="";
			String topicmap="";
			foreach(IParameter p in FrmResumenContenido.resultados)
			{
				if(p.Name.Equals("id"))
				{
					idcont=p.Value;
			...
			try
			{
				...
				XmlDocument docatt=s.getContentAtt(topicmap,topicid,idcont);
				if(docatt.GetElementsByTagName("lastversion").Count>0)
				{
					
					FrmPreview frm=new FrmPreview(topicmap,topicid,idcont, docatt.GetElementsByTagName("lastversion")[0].InnerText,this.admin); 			
