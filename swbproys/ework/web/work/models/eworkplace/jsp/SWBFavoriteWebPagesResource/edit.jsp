...
%>
        <div id="fv_<%=paramRequest.getWebPage().getId()%>">
            <input type="button" value="" class="bkmrk" onclick="postHtml('<%=url%>','fv_<%=id%>')" title="Agregr como favoritos"/>
        </div>
<%                        
    }
    else
...
%>
        <div id="fv_<%=paramRequest.getWebPage().getId()%>">
            <input type="button" value="" class="unbkmrk" onclick="postHtml('<%=url%>','fv_<%=id%>')" title="Quitar de favoritos"/>
        </div>
<%
    }
%>