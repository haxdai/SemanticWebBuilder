...
%><%@page contentType="text/html" pageEncoding="ISO-8859-1"%><jsp:useBean id="paramRequest" scope="request" type="org.semanticwb.portal.api.SWBParamRequest"/><%
            ...
            if (zip != null && !zip.equals("")) {
                ...
                Iterator<Colonia> itco = Colonia.ClassMgr.listColoniaByCpInv(CP.ClassMgr.getCP(col, ws), ws);
%><select name="addrCol" id="addrCol" dojoType="dijit.form.FilteringSelect" required="true" promptMessage="<%=paramRequest.getLocaleString("promptMsgAddrCol")%>">
    ...
    %></select>
    <%
    } else if (mun != null && !mun.equals("")) {
        Iterator<Municipio> itmu = Municipio.ClassMgr.listMunicipioByCP(CP.ClassMgr.getCP(mun, ws), ws);
        if (itmu.hasNext()) {
...
} else if (sta != null && !sta.equals("")) {
    Iterator<Municipio> itmu = Municipio.ClassMgr.listMunicipioByCP(CP.ClassMgr.getCP(sta, ws), ws);
    if (itmu.hasNext()) {
        Municipio municipio = itmu.next();
