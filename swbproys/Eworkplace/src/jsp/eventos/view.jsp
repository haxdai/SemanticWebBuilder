...
<%!
    ...
    public List<Event> getEvents(List<Event> eventos, int year, int month)
    {
        ArrayList<Event> getYears = new ArrayList<Event>();
        for (Event event : eventos)
        {
...
<%
    ...
    gc = new GregorianCalendar(gc.get(Calendar.YEAR), m, gc.get(Calendar.DAY_OF_MONTH),0,0,-1);
    Date di = gc.getTime();
    
    List<Event> allEventsOnMonth = getEvents(events, currentYear, moy.ordinal());
    ...
    if (eventsOnMonth.isEmpty())
    {
        ...
        {
            List<Event> tempEvents = getEvents(events, currentYear, imonth);
    ...
    if (months.size() > 0)
    {
        ...
        for(int range=0; range<events.size()&&range<2; range++)
        {
            Event event = events.get(range);
            out.println("<div>");
