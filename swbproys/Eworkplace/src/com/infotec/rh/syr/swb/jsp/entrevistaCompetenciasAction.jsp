    ...
    if (candidato != null) {
        ...
        if (request.getParameter("apego") != null) {
            try {
                ...
                sc.setPorcentajeCompetencias(ap);
            } catch (Exception e) {
                sc.setPorcentajeCompetencias(0f);
                e.printStackTrace();
            }
        }
        
        if (request.getParameter("nota") != null) {
            sc.setNotasCompetencia(request.getParameter("nota"));
        }
        if (request.getParameter("notaExp") != null) {
            sc.setNotaExperienciaCompetencias(request.getParameter("notaExp"));
        }
        if (request.getParameter("notaTec") != null) {
            sc.setNotasTecnicoCompetencias(request.getParameter("notaTec"));
        }
        
        //sc.setFechaCompetencias(new Date(System.currentTimeMillis()));
    }
    aresponse.setRenderParameter("suri", suri);
    aresponse.setMode(aresponse.Mode_VIEW);
    request.getSession(true).setAttribute("msg", "reload");
