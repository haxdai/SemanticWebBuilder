...
%>
<script>
    window.processInfoStats = <%= processData %>;
</script>
<%
if (showInstances) {
    %>
    <div class="panel panel-default swbp-tray-chart col-lg-3 col-md-6 col-sm-6 col-xs-12">
            ...
            $(document).ready(function() {
                if (window.c3 && window.c3.version) {
                    var chart = c3.generate({
                        data : {
                            columns: window.processInfoStats.instanceData,
                            type: "pie"
                        },
                        size:{height:200},
                        bindto:document.getElementById("performanceGraph")
                    });
                }
            });
        })();
    //updateChart("#performanceGraph", "<%=paramRequest.getLocaleString("lblInstances")%>()", data);
    </script>
    <%
}
if (showResponse) {
    ...
    <div class="panel panel-default col-lg-3 col-md-6 col-sm-6 col-xs-12 swbp-tray-chart">
        <div class="panel-body swbp-tray-body">
            <div class="img-thumbnail" id="responseTime"></div>
        </div>
    </div>
    <script>
        (function() {
            $(document).ready(function() {
                if (window.c3 && window.c3.version) {
                    var chart = c3.generate({
                        data : {
                            columns: window.processInfoStats.responseData,
                            type: "pie"
                        },
                        size:{height:200},
                        bindto:document.getElementById("responseTime")
                    });
                }
            });
        })();
...
if (showStatus) {
    ...
    <div class="panel panel-default col-lg-3 col-md-6 col-sm-6 col-xs-12 swbp-tray-chart">
        <div class="panel-body swbp-tray-body">
            <div class="img-thumbnail" id="overdueGraph"></div>
        </div>
    </div>
    <script>
        (function() {
            $(document).ready(function() {
                if (window.c3 && window.c3.version) {
                    var chart = c3.generate({
                        data : {
                            columns: window.processInfoStats.statusData,
                            type: "pie"
                        },
                        size:{height:200},
                        bindto:document.getElementById("overdueGraph")
                    });
                }
            });
        })();
...
if (showParticipation) {%>
    <div class="panel panel-default col-lg-3 col-md-6 col-sm-6 col-xs-12 swbp-tray-chart">
         <div class="panel-body swbp-tray-body">
            <div class="img-thumbnail" id="participationGraph"></div>
         </div>
    </div>
    <script>
        (function() {
