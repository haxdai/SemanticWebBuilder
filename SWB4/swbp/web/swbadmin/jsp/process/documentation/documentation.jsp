...
if (itTemplateCont == null || !itTemplateCont.hasNext())  {
    ...
    <%
        ...
        SWBResourceURL urlAdminVersion = paramRequest.getRenderUrl().setCallMethod(SWBResourceURL.Call_DIRECT).setMode(SWPDocumentationResource.MODE_ADMIN_VERSION).setParameter("uridi", di.getURI());
...
<div class="row text-right">
    ...
    <ul class="list-unstyled list-inline">
        ...
        <li>
            <a
                onclick="downloadVersion('<%= SWPDocumentationResource.FORMAT_HTML%>')"
        ...
        <li>
         ...
         <li>   
             <a onclick="showModal('<%=urlAdminVersion.setParameter("idp", idp)%>','<%=paramRequest.getLocaleString("btnAddElement")%>','<%=paramRequest.getLocaleString("msgLoadingElement")%>','<%=paramRequest.getLocaleString("msgLoadError")%>', 'modalDialog');" 
        ...
        <li>
            <a href="<%=paramRequest.getWebPage().getParent().getUrl(lang)%>?idpg=<%=idpg%>" class="btn btn-default swbp-btn-action" data-toggle="tooltip" data-placement="bottom" data-original-title="<%=paramRequest.getLocaleString("lblBack")%>" title="<%=paramRequest.getLocaleString("lblBack")%>">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover swbp-table">
                            ...
                            <td style="width: 130px;" class="text-right">
                                ...
                                %>
                                <a class="btn btn-sm btn-default" href="<%= captureUrl + "?idp=" + pr.getId() + "&pg=" + pg + "&p=" + pag + "#" + se.getDocumentSectionInst().getId()%>" target="_blank"
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <%--form method="post" class="form-inline">
                                            <div class="form-row">
                                                <textarea class="tinymce" id="<%= act.getURI()%>" name="<%= dsi.getURI()%>">
                                                    <%out.println(descse);%>
                                                </textarea>
                                            </div>
                                        </form--%>
...
<script type="text/javascript">
    <%--
        tinymce.init({
            ...
            toolbar: "save |insertfile bold italic underline strikethrough | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image table | undo redo code | forecolor backcolor emoticons ",
            menubar: false,
            ...
            height: 200,
            plugins: [
                "jbimages fullpage save advlist table contextmenu link image textcolor code "
            ],
            save_onsavecallback: function(ed) {
                saveText('<%= urlText%>', ed.getContent(), ed.id, 'linotification', '');
            },
            paste_preprocess: function(plugin, args) {
                console.log('the paste 1 : ' + args.content);
    ...
    $(document).ready(function() {
        if (location.hash.slice(1) !== '' && $('#dsitab' + location.hash.slice(1)).length) {
            $('#tabcontainerdsi a[href="#dsitab' + location.hash.slice(1) + '"]').tab('show');
        } else {
...
<form id="svgform" accept-charset="utf-8" method="post" action="<%= urlZip%>">
    ...
    <script src="<%= SWBPortal.getWebWorkPath() %>/models/<%= site.getId() %>/js/bootstrap-colorpicker.min.js"></script>
