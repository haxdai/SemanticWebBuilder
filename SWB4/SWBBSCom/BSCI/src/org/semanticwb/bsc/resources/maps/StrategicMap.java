...
public class StrategicMap extends GenericResource
{
    ...
    public static final int HEADER_6 = (int)(HEADER_1*.50);    // tamaño de fuente para objetivos
...
System.out.println("fin");
    ...
    public String getSvg(HttpServletRequest request) throws XPathExpressionException
    {
        ...
        SVGjs.append(" var marker = document.createElementNS(SVG_, 'marker');").append("\n");
        SVGjs.append(" marker.setAttributeNS(null,'id', 'arrow_1');").append("\n");
        SVGjs.append(" marker.setAttributeNS(null,'viewBox', '0 0 8 8');").append("\n");
        SVGjs.append(" marker.setAttributeNS(null,'refX', '0');").append("\n");
        SVGjs.append(" marker.setAttributeNS(null,'refY', '4');").append("\n");
        ...
        SVGjs.append(" var arrow = document.createElementNS(SVG_, 'path');").append("\n");
        SVGjs.append(" arrow.setAttributeNS(null,'d', 'M0 0 L8 4 L0 8 z');").append("\n");
...
//            SVGjs.append(" g.insertBefore(rect,txt);").append("\n");
            ...
            if (node != null && node.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (attrs.getNamedItem("src").getNodeValue().isEmpty()) {
                    ...
                    SVGjs.append(" img.setAttributeNS(XLINK_,'xlink:href', '" + request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + attrs.getNamedItem("src").getNodeValue() + "');").append("\n");
                    ...
                    if (nodeT != null && nodeT.getNodeType() == Node.ELEMENT_NODE) {
                        ...
                        for (int m = 0; m < nlObjs.getLength(); m++)
                        {
                            ...
                            if (nodeO != null && nodeO.getNodeType() == Node.ELEMENT_NODE)
                            {
                                ...
                                SVGjs.append(" tspan.setAttributeNS(null,'style','fill:#318bc7;font-weight:bold;font-size:"+HEADER_6+"px;');").append("\n");
                    ...
                    if (nodeO != null && nodeO.getNodeType() == Node.ELEMENT_NODE)
                    {
                        ...
                        for (int n = 0; n < nlRels.getLength(); n++)
                        {
                            ...
                            if (nodeR != null && nodeR.getNodeType() == Node.ELEMENT_NODE)
                            {
                                ...
                                SVGjs.append(" if(to && parent) {").append("\n");
                                SVGjs.append("   to.addEventListener('mouseover', remark, false);").append("\n");
                                SVGjs.append("   to.addEventListener('mouseout', forget, false);").append("\n");
                                SVGjs.append("   matxTo = parent.getCTM();").append("\n");
                                SVGjs.append("   posTo = svg.createSVGPoint();").append("\n");
                                SVGjs.append("   posTo.x = to.x.baseVal.value + w;").append("\n");
                                SVGjs.append("   posTo.y = to.y.baseVal.value;").append("\n");
...
expression = "//perspective[@id='" + pid + "']/rel";
NodeList nlRels = (NodeList) xPath.compile(expression).evaluate(map, XPathConstants.NODESET);
for (int n=0; n<nlRels.getLength(); n++)
//if(nlRels.getLength()>0)
{
    Node nodeR = nlRels.item(n);
    //Node nodeR = nlRels.item(0);
    if (nodeR != null && nodeR.getNodeType() == Node.ELEMENT_NODE)
    {
        attrs = nodeR.getAttributes();
        String to = attrs.getNamedItem("to").getNodeValue();
        SVGjs.append(" from = document.getElementById('").append(pid).append("_rct');").append("\n");
        SVGjs.append(" to = document.getElementById('").append(to).append("_rct');").append("\n");

        SVGjs.append("     w = "+pw+";");
        SVGjs.append("     w = w/2;").append("\n");
        SVGjs.append(" if(to && from) {").append("\n");
        SVGjs.append("   matxTo = to.getCTM();").append("\n");
        SVGjs.append("   posTo = svg.createSVGPoint();").append("\n");
        SVGjs.append("   posTo.x = to.x.baseVal.value + w;").append("\n");
        SVGjs.append("   posTo.y = to.y.baseVal.value+to.height.baseVal.value;").append("\n");
        SVGjs.append("   posTo = posTo.matrixTransform(matxTo);").append("\n");
        SVGjs.append("   matxFrm = g.getCTM();").append("\n");
        SVGjs.append("   posFrm = svg.createSVGPoint();").append("\n");
        SVGjs.append("   posFrm.x = from.x.baseVal.value;").append("\n");
        ...
        SVGjs.append("   path = createPath(posFrm.x+'_'+posFrm.y+'_'+posTo.x+'_'+posTo.y,posTo.x,(posFrm.y-7),posTo.x,(posTo.y+2),'#9854A9','#b786c3', 1,60);").append("\n");
        //SVGjs.append("   svg.appendChild(path);").append("\n");
...
SVGjs.append("console.log('color='+color);").append("\n");
        ...
        SVGjs.append("  arrow.addEventListener('click', remark, false);").append("\n");
