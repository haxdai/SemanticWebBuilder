...
<%
    ...
    for(State state:states) {
        ...
        out.println("\">");
        out.println(state.getDisplayTitle(lang));
        out.print("</span>");
    }
    out.println("   </div>");
    out.println("  </div> <!-- /.row -->");
    ...
    table.append(" <table class=\"table  tabla-mapa-impacto\">").append("\n");
    table.append("  <thead>").append("\n");
    table.append("   <tr>").append("\n");
    table.append("    <th>"+paramRequest.getLocaleString("lblPerspective")+"</th>").append("\n"); // Perspectivas
    table.append("    <th>"+paramRequest.getLocaleString("lblObjective")+"</th>").append("\n"); // Objetivos
    for(Initiative initiative : initiatives) {
        if(!initiative.canView()) {
            continue;
        }
        table.append("<th class=\"thimpact\">");
        table.append(initiative.getDisplayTitle(lang));
    ...
    for(int i=0; i<perspectives.size(); i++) {
        ...
        String pTitle = p.getDisplayTitle(lang).replaceAll("['\n]", "");
        ...
        table.append("<td ");
        table.append(" class=\"swbstrgy-imap-obj-").append(i%4).append("\"");
        table.append(">");
table.append("<span class=\"");
table.append(obj.getStatusIconClass(period));
table.append("\"></span>&nbsp;");
        table.append("<a href=\"").append(these.getUrlObjectivePage()).append("?suri=").append(obj.getEncodedURI()).append("\">");
        table.append(obj.getDisplayTitle(lang));
        table.append("</a>");
        table.append("</td>").append("\n");
        for(Initiative initiative:initiatives) {
