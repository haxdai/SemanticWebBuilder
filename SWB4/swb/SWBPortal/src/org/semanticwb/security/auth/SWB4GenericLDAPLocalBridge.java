 ...
 * @author serch
 */
public class SWB4GenericLDAPLocalBridge extends ExtUserRepInt{
    /** The log. */
    static Logger log = SWBUtils.getLogger(SWB4GenericLDAPLocalBridge.class);
     ...
     */
    public SWB4GenericLDAPLocalBridge(UserRepository UserRep, Properties props)
    {
    ...
    public boolean validateCredential(String login, Object credential) 
    {
        ...
        if (login.indexOf('@')>-1){
            ...
            try {
                ret = user.getPassword().equals(SWBUtils.CryptoWrapper.comparablePassword(new String((char[]) credential), alg));
            } catch (GeneralSecurityException se){
                log.error("Can't Atuhenticate", se);
                ret = false;
            }
        } else {
            ret = AuthenticateLP(login, credential);
        }
        return ret;
    }
    ...
    public boolean syncUser(String login, User user)
    {
        ...
        if (login.indexOf('@')>-1) return true;
        boolean ret = false;
        try
        {
